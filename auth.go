...
func New(authConfig *AuthConfig) *Auth {
	...
	if err = migrations.RunCoreMigrations(
		...
		authConfig.Config.Database.URL,
	); err != nil {
...
func (auth *Auth) RunCoreMigrations(ctx context.Context) error {
	return migrations.RunCoreMigrations(ctx, auth.logger, auth.config.Database.Provider, auth.config.Database.URL)
}

func (auth *Auth) DropCoreMigrations(ctx context.Context) error {
	return migrations.DropCoreMigrations(ctx, auth.logger, auth.config.Database.Provider, auth.config.Database.URL)
}
