...
func TestSendEmailVerificationUseCase(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name == "creates verification and calls hook" {
				f.mailerSvc.AssertNotCalled(t, "SendEmail", mock.Anything, "test@example.com", mock.Anything, mock.Anything, mock.Anything)
			}
		})
	}
}
