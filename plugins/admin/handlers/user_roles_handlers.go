...
func (h *ReplaceUserRolesHandler) Handler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		reqCtx.SetJSONResponse(http.StatusOK, map[string]any{"message": "user roles replaced"})
	}
}
...
func (h *AssignUserRoleHandler) Handler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		reqCtx.SetJSONResponse(http.StatusOK, map[string]any{"message": "role assigned"})
	}
}
...
func (h *RemoveUserRoleHandler) Handler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		reqCtx.SetJSONResponse(http.StatusOK, map[string]any{"message": "role removed"})
	}
}
...
func respondUserHandlerError(reqCtx *models.RequestContext, err error) {
	...
	reqCtx.SetJSONResponse(mapUserHandlerErrorStatus(err), map[string]any{"message": message})
	reqCtx.Handled = true
}
...
func mapUserHandlerErrorStatus(err error) int {
	...
	case strings.Contains(message, "required"), strings.Contains(message, "invalid"), strings.Contains(message, "cannot"), strings.Contains(message, "expires"):
