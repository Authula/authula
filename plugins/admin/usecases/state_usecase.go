...
func NewStateUseCase(userStateService *services.UserStateService, sessionStateService *services.SessionStateService, impersonation *services.ImpersonationService) StateUseCase {
	return &stateUseCase{userStateService: userStateService, sessionStateService: sessionStateService, impersonation: impersonation}
}
...
func (u *stateUseCase) UpsertUserState(ctx context.Context, userID string, request types.UpsertUserStateRequest, actorUserID *string) (*types.AdminUserState, error) {
	...
	exists, err := u.impersonation.UserExists(ctx, userID)
	if err != nil {
		return nil, err
	}
	if !exists {
	...
	if err := u.userStateService.Upsert(ctx, state); err != nil {
		return nil, err
	}

	return u.userStateService.GetByUserID(ctx, userID)
}
...
func (u *stateUseCase) UpsertSessionState(ctx context.Context, sessionID string, request types.UpsertSessionStateRequest, actorUserID *string) (*types.AdminSessionState, error) {
	...
	exists, err := u.sessionStateService.SessionExists(ctx, sessionID)
	if err != nil {
		return nil, err
	}
	if !exists {
	...
	return u.sessionStateService.GetBySessionID(ctx, sessionID)
}
...
func (u *stateUseCase) GetUserAdminSessions(ctx context.Context, userID string) ([]types.AdminUserSession, error) {
	...
	return u.sessionStateService.GetByUserID(ctx, userID)
}
